...
**Контекст:** Раскатываем агентов (Hermes + Multica) на всю компанию. MCP-сервер sphere-tasks сейчас работает с одним пользователем (webhook hardcoded в env). Нужна архитектура для 50–500 сотрудников.
...
- **sphere-tasks:** `userId` как параметр tool call (~20 строк)
